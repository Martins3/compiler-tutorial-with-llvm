...
// 4. 在逐级构建expr : *(a + 4)
//    5. wait ! 实际上，只有在 unary 的时候开始检查，然后在连续的变化中间，始终维持heap 和 地址
//    6. *(a + 4) 的记录 a + 4 为地址和 *(a +4) 为值向上传递
//    7. *(a + 4) + 4  直接导致清空当前的 heap 记录表，其实根本不需要一个表格 ? 其实只需要一个变量即可!
//    8. 遇到 * 的时候建立刷新 address 的
//    9. *a = *a + 1 会出现问题，右侧的递归处理会掩盖左侧的内容，所以还是最好建立一个映射表
...
public:
   ...
   void decl(DeclStmt * declstmt) {
	   for (DeclStmt::decl_iterator it = declstmt->decl_begin(), ie = declstmt->decl_end();
         ...
         // if(type->isArrayType()) {
          ...
          // std::cout << "Is array of type: "  << Array->getElementType().getAsString() << std::endl;
          // std::cout << "Is array of type: "  << Array->getSizeModifier() << std::endl;
          // std::cout << vardecl->getType()->isVariableArrayType() << std::endl;
         // }
         ...
         }else if (auto t =  dyn_cast_or_null<VariableArrayType>(type.getTypePtr())) {  
           t->getSizeExpr()->dump();
   ...
   void declref(DeclRefExpr * declref) {
       ...
       // TODO 没有对应的调整和变化吗? 这是不是顺便处理了函数指针的问题 ?
		   Decl* decl = declref->getFoundDecl();
		   lld val = mStack.back().getDeclVal(decl);
		   mStack.back().bindStmt(declref, val);
     }
   ...
   void cast(CastExpr * castexpr) {
       ...
       // 地址和普通的数值根本无法区分管理，而且地址变量本来也就是stack frame 管理的东西
       ...
       // TODO heap 应该如何管理的 ?
       // 函数指针类型的变化我们无需处理
       // TODO 我觉得应该不需要这一个检查才对啊!
       if(castexpr->getType()->isFunctionPointerType() == true) {
         fprintf(stderr, "%s\n", "Oh, come across a cute function pointer implicit cast, just let it go !");
         return;
       }
